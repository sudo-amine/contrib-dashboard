    ...
    return () => {
      ...
      return h("div", { class: "wst-node" }, [
        h("div", {
          ...
          "aria-expanded": showChevron ? n.expanded : undefined,
          "data-workspace-path": n.path,
          ...
          showChevron
            ? h("button", {
                type: "button",
                class: ["wst-chev", n.expanded ? "open" : ""],
                title: n.expanded ? "Collapse workspace" : "Expand workspace",
                "aria-label": n.expanded ? `Collapse ${n.name}` : `Expand ${n.name}`,
...
// Already-bound export names in the current workspace (to filter them out)
...
async function openModal() {
  ...
  wsTree.value = [rootNode];
  await loadWsChildren(rootNode);
...
async function confirmWorkspace() {
  ...
  try {
    const list = await listAPIExports(path);
    const all = list.items ?? [];
    availExports.value = all.filter(ex => {
      const key1 = path + "/" + ex.metadata.name;
      const key2 = ex.metadata.name;
      return !boundExportKeys.value.has(key1) && !boundExportKeys.value.has(key2);
    });
  } catch { availExports.value = []; }
  finally { exportsLoading.value = false; }
}

// Highlight an export row (click) — does NOT confirm yet
...
async function create() {
  ...
  try {
    ...
    // Keep syncing through reconciliation, not merely until the POST result is
    // visible in a list response.
    void pollUntil(
      list => list.some(x => x.metadata.name === createdName && apiBindingStatus(x).ready),
      cluster,
    );
  } catch (e) {
...
function resetModal() {
  selectedWs.value = ""; highlightedWs.value = ""; wsTree.value = [];
...
// ── List + polling ───────────────────────────────────────────────────────────
async function fetchBindings(cluster = current.value) {
...
async function load() {
  ...
  try {
    const list = await fetchBindings(cluster);
    if (request === loadRequest && cluster === current.value) items.value = list;
  } catch (e) {
...
async function pollUntil(done: (list: APIBinding[]) => boolean, cluster = current.value) {
  if (cluster !== current.value) return;
  poller?.stop();
  const currentPoller = usePoller(
    async () => {
      const list = await fetchBindings(cluster);
      if (poller === currentPoller && cluster === current.value) items.value = list;
      return list;
    },
    done,
  ...
  if (poller === currentPoller) pollStatus.value = currentPoller.state.value;
}
...
async function doDelete() {
  ...
  try {
    await deleteAPIBinding(cluster, deletedName);
    confirmBinding.value = null;
    // Poll until binding is gone
    void pollUntil(list => !list.some(x => x.metadata.name === deletedName), cluster);
  } catch (e) {
    error.value = e instanceof ApiError ? e.message : String(e);
    confirmBinding.value = null;
        ...
        <div v-if="showModal" class="modal-backdrop" @click.self="closeModal">
          <div class="modal" role="dialog" aria-modal="true">
            ...
            <div class="modal-head">
              ...
              <button class="modal-close" title="Close" aria-label="Close binding wizard" @click="closeModal">
            ...
            <div class="modal-body">
              ...
              <div class="step" :class="{ disabled: !selectedWs }">
                ...
                <div v-if="selectedWs && !selectedExport" class="export-picker">
                  ...
                  <template v-else>
                    <p v-if="!availExports.length" class="picker-hint">No unbound exports in this workspace</p>
                    <template v-else>
                      <div class="export-list">
                        <button
                          v-for="ex in availExports" :key="ex.metadata.name"
                          class="export-option" :class="{ 'export-option--active': highlightedExport === ex }"
...
.export-option:hover { border-color: #c7d2fe; background: #fafbff; }
